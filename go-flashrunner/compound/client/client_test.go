...
func Test_GetLiq(t *testing.T) {
	client :=NewClient(api)
	resp,err:=client.GetUnhealthyAccount("1.10","4")
	if(err!=nil){
		fmt.Println("this is err,",resp)
	}
	fmt.Println(resp)
}
func Test_Client(t *testing.T) {
	...
	if accts, err := client.GetLiquidatableAccounts("50", "1"); err != nil {
		t.Fatal(err)
	} else if len(accts) == 0 {
