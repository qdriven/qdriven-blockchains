...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		d = td.Delay()
		t.signal.Reset(d)
		debug("timer: add reset delay %d ms", int64(d)/int64(time.Millisecond))
	}
	debug("timer: push item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
}
...
func (t *Timer) del(td *TimerData) {
	...
	if i < 0 || i > last || t.timers[i] != td {
		...
		debug("timer del i: %d, last: %d, %p", i, last, td)
		return
	}
	if i != last {
		t.swap(i, last)
	...
	debug("timer: remove item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
}
...
func (t *Timer) expire() {
	...
	for {
		...
		t.lock.Unlock()
		if fn == nil {
			debug("expire timer no fn")
		} else {
			debug("timer key: %s, expire: %s, index: %d expired, call fn", td.Key, td.ExpireString(), td.index)
			fn()
		}
		t.lock.Lock()
	}
	t.signal.Reset(d)
	debug("timer: expier reset delay %d ms", int64(d)/int64(time.Millisecond))
	t.lock.Unlock()
}
